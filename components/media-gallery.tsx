...
export function MediaGallery() {
  ...
  if (loading) return <div className="flex items-center justify-center py-12"><p className="text-foreground/60">Loading media...</p></div>
  if (media.length === 0) return <div className="py-12 text-center"><p className="text-lg text-foreground/60">No media available yet.</p></div>
  ...
  const card = (item: Media, featuredCard = false) => (
    <Link
      ...
      className={`group relative block w-full overflow-hidden rounded-lg ${featuredCard ? 'aspect-video' : 'aspect-square'}`}
